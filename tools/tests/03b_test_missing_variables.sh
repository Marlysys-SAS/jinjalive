...
echo '
# ###########################################
# Testing: 03b_test_missing_variables.sh
# ###########################################
'
...
#  
#  Run : ./03b_test_missing_variables.sh
#
########################################################
...
# Launch test
jinjalive -cf sample_context.yaml -tf sample_template_missing_variables_2.tpl -of sample_output.out
RETVAL=$?
[ $RETVAL -eq 0 ] && echo Failure: Test 'missing variable' 2 should raise an Exception, but no exception raised
[ $RETVAL -ne 0 ] && echo Success: Test 'missing variable' 2 raised an Exception and error code is not 0
set -e
